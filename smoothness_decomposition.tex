...
\begin{remark}[Realization as a difference set]
Theorem~\ref{thm:main} produces a covering \emph{set} $D$, not a difference set
$S-T$. We caution that $D$ is not realizable as the difference set of two small
balanced sets: prescribing the $|D|=\pi(n)(1+o(1))$ specific values
$L,\,Lq_1,\,Lq_2,\dots$ as differences $s_i-t_j$ imposes $|D|$ constraints on
the $2K$ values in $S\cup T$, which is infeasible for $K\ll|D|$. Thus obtaining
a \emph{balanced} covering pair $(S,T)$ with $|S|,|T|=K$ from a covering set is a
genuinely separate problem; the trivial realization $S=D,\ T=\{0\}$ gives only
$K=|D|$. We make no claim here about the least $K$ for which a balanced covering
pair exists.
\end{remark}
